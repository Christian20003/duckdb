...
ScalarFunctionSet NextAfterFun::GetFunctions() {
	...
	next_after_fun.AddFunction(ScalarFunction({LogicalType::HALF_FLOAT, LogicalType::HALF_FLOAT}, LogicalType::HALF_FLOAT,
...
template <>
int8_t SignOperator::Operation(std::bfloat16_t input) {
...
ScalarFunctionSet CeilFun::GetFunctions() {
	...
	for (auto &type : LogicalType::Numeric()) {
		...
		case LogicalTypeId::HALF_FLOAT:
			func = ScalarFunction::UnaryFunction<std::bfloat16_t, std::bfloat16_t, CeilOperator>;
			break;
...
ScalarFunctionSet FloorFun::GetFunctions() {
	...
	for (auto &type : LogicalType::Numeric()) {
		...
		switch (type.id()) {
		case LogicalTypeId::HALF_FLOAT:
			func = ScalarFunction::UnaryFunction<std::bfloat16_t, std::bfloat16_t, FloorOperator>;
			break;
		case LogicalTypeId::FLOAT:
...
ScalarFunctionSet TruncFun::GetFunctions() {
	...
	for (auto &type : LogicalType::Numeric()) {
		...
		switch (type.id()) {
		case LogicalTypeId::HALF_FLOAT:
			func = ScalarFunction::UnaryFunction<std::bfloat16_t, std::bfloat16_t, TruncOperator>;
			break;
		case LogicalTypeId::FLOAT:
...
ScalarFunctionSet RoundFun::GetFunctions() {
	...
	for (auto &type : LogicalType::Numeric()) {
		...
		switch (type.id()) {
		case LogicalTypeId::HALF_FLOAT:
			round_func = ScalarFunction::UnaryFunction<std::bfloat16_t, std::bfloat16_t, RoundOperator>;
			round_prec_func = ScalarFunction::BinaryFunction<std::bfloat16_t, int32_t, std::bfloat16_t, RoundOperatorPrecision>;
			break;
		case LogicalTypeId::FLOAT:
...
ScalarFunctionSet IsNanFun::GetFunctions() {
	ScalarFunctionSet funcs;
	funcs.AddFunction(ScalarFunction({LogicalType::HALF_FLOAT}, LogicalType::BOOLEAN,
									 ScalarFunction::UnaryFunction<std::bfloat16_t, bool, IsNanOperator>));
...
ScalarFunctionSet SignBitFun::GetFunctions() {
	ScalarFunctionSet funcs;
	funcs.AddFunction(ScalarFunction({LogicalType::HALF_FLOAT}, LogicalType::BOOLEAN,
									 ScalarFunction::UnaryFunction<std::bfloat16_t, bool, SignBitOperator>));
	funcs.AddFunction(ScalarFunction({LogicalType::FLOAT}, LogicalType::BOOLEAN,
...
ScalarFunctionSet IsInfiniteFun::GetFunctions() {
	...
	funcs.AddFunction(ScalarFunction({LogicalType::HALF_FLOAT}, LogicalType::BOOLEAN,
									 ScalarFunction::UnaryFunction<std::bfloat16_t, bool, IsInfiniteOperator>));
	funcs.AddFunction(ScalarFunction({LogicalType::FLOAT}, LogicalType::BOOLEAN,
...
ScalarFunctionSet IsFiniteFun::GetFunctions() {
	ScalarFunctionSet funcs;
	funcs.AddFunction(ScalarFunction({LogicalType::HALF_FLOAT}, LogicalType::BOOLEAN,
									 ScalarFunction::UnaryFunction<std::bfloat16_t, bool, IsFiniteOperator>));
	funcs.AddFunction(ScalarFunction({LogicalType::FLOAT}, LogicalType::BOOLEAN,
