...
Value Value::MinimumValue(const LogicalType &type) {
	...
	case LogicalTypeId::BFLOAT:
		return Value::BFLOAT(NumericLimits<std::bfloat16_t>::Minimum());
...
Value Value::MaximumValue(const LogicalType &type) {
	...
	case LogicalTypeId::BFLOAT:
		return Value::BFLOAT(NumericLimits<std::bfloat16_t>::Maximum());
	case LogicalTypeId::FLOAT:
...
Value Value::Infinity(const LogicalType &type) {
	...
	case LogicalTypeId::BFLOAT:
		return Value::BFLOAT(std::numeric_limits<std::bfloat16_t>::infinity());
	case LogicalTypeId::FLOAT:
...
Value Value::BFLOAT(std::bfloat16_t value) {
	Value result(LogicalType::BFLOAT);
	result.value_.bfloat = value;
	result.is_null = false;
	return result;
}
...
T Value::GetValueInternal() const {
	...
	case LogicalTypeId::BFLOAT:
		return Cast::Operation<std::bfloat16_t, T>(value_.bfloat);
	case LogicalTypeId::FLOAT:
...
Value Value::Numeric(const LogicalType &type, int64_t value) {
	...
	case LogicalTypeId::BFLOAT:
		return Value::BFLOAT((std::bfloat16_t) value);
	case LogicalTypeId::FLOAT:
		return Value((float)value);
	case LogicalTypeId::DOUBLE:
		return Value((double)value);
...
std::bfloat16_t Value::GetValueUnsafe() const {
	D_ASSERT(type_.InternalType() == PhysicalType::BFLOAT);
	return value_.bfloat;
}
...
void Value::SerializeInternal(Serializer &serializer, bool serialize_type) const {
	...
	case PhysicalType::BFLOAT:
		serializer.WriteProperty(102, "value", value_.bfloat);
		break;
...
Value Value::Deserialize(Deserializer &deserializer) {
	...
	case PhysicalType::INT128:
		...
		break;
	case PhysicalType::BFLOAT:
		new_value.value_.bfloat = deserializer.ReadProperty<std::bfloat16_t>(102, "value");
		break;
	case PhysicalType::FLOAT:
